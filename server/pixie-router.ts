...
function buildSuggestUserMessage(input: {
  ...
  if (input.hintMode) {
    parts.push(`## Suggested Mode\nThe user has selected: **${input.hintMode}**. Prioritize this mode in your analysis unless the message clearly calls for a different approach.`);
  }
  parts.push(`\nAnalyze the user's raw message through the five-layer analysis (Surface Message → True Intent → Emotion State → Social Risk → User Voice), ${input.hintMode ? `treating the suggested mode as the primary lens,` : "auto-detect the best mode,"} and generate the response.`);
...
export const pixieRouter = router({
  ...
  suggest: publicProcedure
    .input(
      z.object({
        ...
        persona: personaSchema.default("lumi"),
        rawMessage: z.string(),
        hintMode: suggestionModeSchema.optional(),
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        return {
          ...
          alternatives: { playful: "", softer: "", casual: "" },
          userVoiceMatch: 0,
      ...
      return {
        ...
        privateBubbles: Array.isArray(parsed.privateBubbles) ? parsed.privateBubbles.map((b: any) => ({
          type: ["reaction", "tease", "advice", "warning", "question"].includes(b.type) ? b.type : "advice",
        ...
        alternatives: {
          ...
          softer: typeof parsed.alternatives?.softer === "string" ? parsed.alternatives.softer : "",
          casual: typeof parsed.alternatives?.casual === "string" ? parsed.alternatives.casual : "",
        },
    ...
    .mutation(async ({ input }) => {
      ...
      try {
        ...
        return {
          ...
          content: raw,
          bubbles: Array.isArray(parsed.bubbles) ? parsed.bubbles.map((b: any) => ({
            type: ["reaction", "tease", "comfort", "advice", "question", "action", "warning"].includes(b.type) ? b.type : "advice",
            text: typeof b.text === "string" ? b.text : "",
  ...
  // --- Public Speak: in-chat pixie as open conversation participant ------
  publicSpeak: publicProcedure
    .input(
      z.object({
        persona: personaSchema.default("lumi"),
        pixieName: z.string().default("Lumi"),
        ownerName: z.string().default("Lucy"),
        otherName: z.string().default("Pat"),
        chatContext: z.array(chatMessageSchema).default([]),
        triggerMessage: z.string(),
        triggerBy: z.enum(["owner", "other"]),
      })
    )
    .mutation(async ({ input }) => {
      const systemPrompt = assemblePublicSpeakPrompt(
      ...
      const contextLines = input.chatContext
        .slice(-8)
      ...
      const userMessage = [
        "## Recent Conversation",
        contextLines || "(conversation start)",
        "",
        `## Message that triggered this (sent by ${input.triggerBy === "owner" ? input.ownerName : input.otherName})`,
        input.triggerMessage,
        "",
        `Now as ${input.pixieName}, decide whether to speak publicly and what to say.`,
      ].join("\n");
